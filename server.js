...
function seoLayout({title,description,canonical,image,body,jsonLd='',verification=''}){return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escHtml(title)}</title><meta name="description" content="${escHtml(description)}"><link rel="canonical" href="${escHtml(canonical)}"><meta name="robots" content="index,follow,max-image-preview:large"><meta property="og:type" content="website"><meta property="og:site_name" content="Blog24 Pi Shopping"><meta property="og:title" content="${escHtml(title)}"><meta property="og:description" content="${escHtml(description)}"><meta property="og:url" content="${escHtml(canonical)}"><meta property="og:image" content="${escHtml(image)}"><meta name="twitter:card" content="summary_large_image">${verification||''}${jsonLd?`<script type="application/ld+json">${jsonLd.replace(/<\//g,'<\\/')}</script>`:''}<link rel="stylesheet" href="/style.css?v=1.6.19"></head><body>${body}</body></html>`;}
...
async function effectiveProducts(){
 ...
 try{
  ...
  return r.rows.map(x=>({id:x.id,productCode:x.product_code||`BLOG24-P-${String(x.id).padStart(6,'0')}`,name:x.name,price:Number(x.price),shipping:Number(x.shipping||0),shippingText:x.shipping_text||'',category:x.category||'기타',desc:x.description||'',detail:x.detail||'',options:x.options||['기본'],images:x.images||[],detailImages:x.detail_images||[],youtube:x.youtube_url||'',active:x.active!==false,sortOrder:x.sort_order||x.id,stock:Number.isFinite(Number(x.stock))?Number(x.stock):9999}));
...
async function ensureV16Schema(){
  ...
  await pool.query(`UPDATE products SET product_code='BLOG24-P-' || LPAD(id::text,6,'0') WHERE product_code IS NULL OR product_code=''`);
