...
async function openMyOrders(){
 ...
 try{
  ...
  box.innerHTML=d.orders.map(o=>{const pending=o.order_status==="PENDING";const exp=new Date(o.pending_expires_at||new Date(new Date(o.ordered_at).getTime()+CART_TTL_MS)).getTime();const track=o.tracking_number?`<div class="order-meta">${o.courier||"택배"} · <button class="tracking-link" data-courier="${o.courier||""}" data-track="${o.tracking_number}">송장 ${o.tracking_number} 조회</button></div>`:"";const actions=`<div class="order-actions">${pending?`<button class="retry-pay" data-order="${o.order_id}">다시 결제하기</button><button class="delete-pending" data-order="${o.order_id}">주문 삭제</button>`:""}${o.order_status==="DELIVERED"?`<button class="confirm-buy" data-order="${o.order_id}">구매확정</button>`:""}</div>`;return `<div class="order-card"><div class="order-card-head"><b>${o.order_id}</b><span class="order-status">${statusKo(o.order_status)}</span></div><small>${fmtDate(o.ordered_at)} · ${Number(o.paid_total).toFixed(2)} π</small>${pending?`<div class="pending-timer" data-exp="${exp}">⏱ ${leftHms(exp-Date.now())} 후 자동 삭제됩니다</div>`:""}<div class="order-items">${(o.items||[]).map(i=>`${i.productName} × ${i.quantity} · ${(Number(i.unitPrice)*Number(i.quantity)).toFixed(2)} π`).join("<br>")}</div>${track}<div class="order-meta">Payment ID: ${o.payment_id||"-"}<br>TXID: ${o.txid||"-"}</div>${actions}</div>`}).join("");
  ...
  box.querySelectorAll(".delete-pending").forEach(b=>b.onclick=async()=>{if(!confirm("결제대기 주문을 삭제하시겠습니까?"))return;try{const r=await fetch(`/api/orders/${encodeURIComponent(b.dataset.order)}/pending`,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({uid:currentUser.uid})});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||"삭제 실패");m.remove();toast("결제대기 주문을 삭제했습니다.");openMyOrders()}catch(e){toast(e.message)}});
...
function openProductDetail(id){
 ...
 m.querySelector("#detailShare").onclick=async()=>{const u=new URL(`/product/${p.id}`,location.origin);const d={title:p.name,text:`${p.name} · ${Number(p.price).toFixed(2)} π`,url:u.toString()};try{if(navigator.share)await navigator.share(d);else{await navigator.clipboard.writeText(d.url);toast("상품 링크를 복사했습니다.");}}catch(e){}};
